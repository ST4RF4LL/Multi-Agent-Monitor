...
function createTerminalWSServer(server) {
  ...
  wss.on('connection', (ws, req) => {
    ...
    // Resize + trigger TUI redraw at correct dimensions
    const syncDimensions = async (cols, rows) => {
      ...
      // Trigger full TUI redraw: clear screen + Ctrl+L refreshes the app
      // \x1b[H\x1b[J clears the terminal view on client side
      sendData('\x1b[H\x1b[J');

      // Send Ctrl+L to the process to force a complete TUI repaint
      // at the new dimensions. The output flows through pipe-pane → client.
    ...
    (async () => {
      ...
      // Start streaming immediately — output at default size until first resize
      try {
        await startPipePane();
        startFifoReader();
      } catch (err) {
      ...
      try {
        if (data.type === 'resize') {
          ...
          if (!resized) {
            ...
            // Subsequent resizes: just resize, TUI will adapt on next redraw
            await tmux.exec([
              'resize-window', '-t', target,
              '-x', String(cols), '-y', String(rows),
            ]).catch(() => {});
